...
class Preprocessor:
    def __init__(self, preprocess_config, model_config, train_config):
        ...
        self.prepro_subdir = preprocess_config["path"]["prepro_subdir"]
        ...
        if self.multi_speaker and preprocess_config["preprocessing"]["speaker_embedder"] != "none":
            self.which_speaker_emb = preprocess_config["preprocessing"]["speaker_embedder"]
    ...
    def build_from_path(self):
        ...
        for i, speaker in enumerate(tqdm(self.in_sub_dirs)):
            ...
            if os.path.isdir(os.path.join(self.in_dir, speaker)):
                speakers[speaker] = i
                for wav_name in tqdm(os.listdir(os.path.join(self.in_dir, speaker, self.prepro_subdir))):
                    if ".wav" not in wav_name:
                        continue

                    basename = wav_name.split(".")[0]
                    tg_path = os.path.join(
                        self.in_dir, speaker, "textgrid", "{}.TextGrid".format(basename)
                    ) #change from outdir to indir   '/666/dsets/L2arc16prepro2/TextGrid/HKK/arctic_b0506.TextGrid'
                    (
                        info_unsup,
                        info_sup,
                        f0_unsup,
    ...
    def process_utterance(self, tg_path, speaker, basename, save_speaker_emb):
        ...
        wav_path = os.path.join(self.in_dir, speaker, self.prepro_subdir, "{}.wav".format(basename)) #change to include prepro_subdir
        text_path = os.path.join(self.in_dir, speaker, self.prepro_subdir, "{}.lab".format(basename))

        wav_raw, wav, duration = self.load_audio(wav_path) #change to
        ...
        if self.which_speaker_emb=='DeepSpeaker':
            spker_embed = self.speaker_emb(wav) if save_speaker_emb else None #DS
        elif self.which_speaker_emb=='GE2E':
